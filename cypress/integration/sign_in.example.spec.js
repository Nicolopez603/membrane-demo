...
describe("Test suite at sign-in", function () {
  ...
  it("Validating that logo and title is visible", function () {
    signInPage.elements.membraneLogo().should("be.visible");

    signInPage.elements.membraneTitle().should("have.text", "Membrane");
  });

  it("Validating that the title contains Sign-in", function () {
    signInPage.elements.signInTitle().should("contain.text", "Sign In");
  });
  ...
  it("Validating that Eye(svg) element at the end on the password field", function () {
    signInPage.elements.eyePass().should("be.visible");
  });

  it("Validating the link sign-up ", function () {
    signInPage.elements.signUpButton().should("be.visible").eq(1).click();

    cy.location("pathname").should("contains", "/sign-up");
  });

  it("Validating the link forgot-password ", function () {
    signInPage.elements.forgotPassButton().should("be.visible");
  });
  ...
  it("Validating the next button is inactive", function () {
    cy.readFile("cypress/fixtures/userCredentials.json")
      ...
      .then(() => {
        signInPage.elements.nextButtonDisabled().should("be.disabled");
      });
  });
  ...
  it("Validating the “Wrong email or password, try again.” on popup message", function () {
    cy.readFile("cypress/fixtures/userCredentials.json")
      ...
      .then(() => {
        ...
        signInPage.elements
          .popUpAlert()
          .should("have.text", "Error: Wrong email or password, try again.");
      });
  });
